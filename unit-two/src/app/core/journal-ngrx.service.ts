...
import { debounceTime, distinctUntilChanged, filter, map, pluck, switchMap, take, tap } from 'rxjs/operators';
...
export class JournalNgrxService implements JournalService {
  ...
  constructor(private storeService: StoreService, private backend: JournalBackendMockService) {
    this.storeService.getState().pipe(
      filter(val => val.journalInfo !== undefined),
      map(({ journalInfo, search, page, pageSize, filters, forceReload }) => [
        { journalId: journalInfo.id, search, page, pageSize, filters, forceReload },
        forceReload,
      ]),
      distinctUntilChanged(([aRequest], [bRequest, forceReload]) => !forceReload && JSON.stringify(aRequest) === JSON.stringify(bRequest)),
      debounceTime(0),
      switchMap(([request, _]: [JournalRequest, any]) => this.backend.getResult(request)),
